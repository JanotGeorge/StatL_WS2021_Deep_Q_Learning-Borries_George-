...
class Snake(object):
    ...
    def Netz_trainieren(self, NW):
        ...
        for Erinnerung in self.trainings_set:   
            q.append(NW(torch.FloatTensor(Erinnerung[0]))[Erinnerung[1]])
            q_stern.append(Erinnerung[2] + gamma * torch.argmax(NW(torch.FloatTensor(Erinnerung[3]))))
...
while Spiel.Episodennummer <= Episodenanzahl:
    ...
    while not Spiel.Abbruch():
        ...
        q_stern, q = Spiel.Netz_trainieren(NW=Strategie_NW)
